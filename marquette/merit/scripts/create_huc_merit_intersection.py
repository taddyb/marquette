...
def create_TM(cfg: DictConfig, gdf: gpd.GeoDataFrame) -> None:
    ...
    for idx, id in enumerate(tqdm(huc10_ids, desc="creating TM", ncols=140, ascii=True,)):
        ...
        if abs(percent_error) > 0.05:
            log.info(f"HUC10: {merit_basins.huc10.iloc[0]} has bad clipping. Merit basins percent error = {percent_error}")
...
def create_intersections(cfg: DictConfig) -> None:
    ...
    albers_conus = CRS.from_proj4("+proj=aea +lat_1=29.5 +lat_2=45.5 +lat_0=23 +lon_0=-96 +x_0=0 +y_0=0 +datum=NAD83 +units=m +no_defs")
